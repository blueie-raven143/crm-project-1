...
def login():
    while True:
        ...
        if user == None:
            ...
            break
    return user[0][0] if user> 0 else user[0]

def getData(bda_id):
    return cursor.execute('SELECT * FROM bda WHERE bda_id = \'{}\''.format(bda_id)).fetchone()
...
def analyze(bda_id):
    ...
    color = ['Orange','Pink','Turquoise','Brown','Gray']
    fig = px.pie(df, values=values, names=names, title='Leads Result')
    fig.update_traces(
    textposition='inside',textinfo='percent+label', marker=dict(colors=color, line=dict(color='#000000', width = 2))
    )
    fig.write_html('lead_result.html', auto_open = True)
    print('Analysis done, opening the results in browser.......')

def checkout(bda_id):
    today = datetime.now().strfttime('%Y - %m - %d')
    result = cursor.execute('SELECT lead_result FROM sales WHERE bda_id = \'{}\' AND lead_status = 1'.format(bda_id)).fetchone()
    rate = 2000
    total_leads = cursor.execute('SELECT count(*) FROM sales WHERE bda_id = \'{}\''.format(bda_id)).fetchone()
    handled_leads = cursor.execute('SELECT count(*) FROM sales WHERE bda_id = \'{}\' AND lead_status = 1'.format(bda_id)).fetchone()
    unhandled_leads = cursor.execute('SELECT count(*) FROM sales WHERE bda_id = \'{}\' AND lead_status = 0'.format(bda_id)).fetchone()
    print("Total leads:", total_leads[0][0])
    print("Handled leads:", handled_leads[0][0])
    print("Unhandled leads:", unhandled_leads[0][0])
...
while True:
    data = getData(bda_id)
    result = divideLeads(data)
    print("----------------------------------------------")
    print("Remaining Leads:{}".format(len(result[1])))
    print("Handled leads: {}".format(len(result[0])))
    print()
    print("----------------------------------------------------------------")
    print()
    print()
    ...
    if ch == 0:
        ...
        current_lead = cursor.execute('SELECT * FROM sales WHERE bda_id = \'{}\' AND lead_status;'.format(result[0][0]))
        if current_lead == None:
            print("No more leads to handle! Congradulations")
            break
        print()
        ...
        print("5. Junk Leads/Wrong Number")
